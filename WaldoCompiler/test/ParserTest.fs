...
let ``Parser should be able to parse a for loop``() = 
    let test1 = 
        ...
        [ makeNamespace "System" [ DefOperator(pos "+", [], pos (Tuple []), [ ForLoop(pos "i", ConstExpr(ConstUInt(pos 0I)), ConstExpr(ConstUInt(pos 42I))) ]) ] ]
    [ test1 ] |> testOnDataMapAttempt parse
...
let ``Parser should be able to parse a method call``() = 
    let test1 = 
        "namespace System { def map(): Unit { malloc(4) }}", 
        [ makeNamespace "System" [ DefFunction(pos "map", [], pos (Tuple []), [ MethodCallStatement(pos "malloc", [ ConstExpr(ConstInt(pos 4I)) ]) ]) ] ]
    let test2 = 
        "namespace System { def map(): Unit { val n = malloc(4) }}", 
        [ makeNamespace "System" 
              [ DefFunction
                    (pos "map", [], pos (Tuple []), [ ValDeclaration(p,  (pos "n", pos Unknown) :: [], MethodCall(pos "malloc", [ ConstExpr(ConstInt(pos 4I)) ])) ]) ] ]
    let test3 = 
        "namespace System { def map(): Unit { 4.malloc() }}", 
        [ makeNamespace "System" [ DefFunction(pos "map", [], pos (Tuple []), [ MethodCallStatement(pos "malloc", [ ConstExpr(ConstInt(pos 4I)) ]) ]) ] ]
    let test4 = 
        "namespace System { def map(): Unit { System:malloc(4) }}", 
        [ makeNamespace "System" [ DefFunction(pos "map", [], pos (Tuple []), [ MethodCallStatement(pos "System;malloc", [ ConstExpr(ConstInt(pos 4I)) ]) ]) ] ]
    [ test1; test2; test3; test4 ] |> testOnDataMapAttempt parse
...
let ``Parser should be able to parse undefined types``() = 
    let test1 = 
        ...
        [ makeNamespace "System" [ DefFunction(pos "map", [], pos Unknown, 
    ...
    let test1 = 
        ...
        [ makeNamespace "System" 
              [ DefFunction
                    (pos "map", [], pos Unknown, 
                     [ VarDeclaration
                           (p, (pos "a", pos (Tuple(Identifier "Int" :: Identifier "Int" :: []))) :: [], 
...
let ``Parser should be able to parse unary minus``() = 
    let test1 = 
        ...
        [ makeNamespace "System" 
              [ DefFunction(pos "map", [], pos Unknown, [ VarDeclaration(p, (pos "a", pos Unknown) :: [], UnaryOperatorCall(pos "-", ConstExpr(ConstInt(pos 4I)))) ]) ] ]
    [ test1 ] |> testOnDataMapAttempt parse
